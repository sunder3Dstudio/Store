    ...
    inputBuscar.addEventListener('input', (e) => {
        ...
        if (coincidencias.length > 0) {
            gridResultados.innerHTML = coincidencias.map(p => {
                // Obtiene la ruta correcta de la imagen según cómo esté guardada en el objeto
                let rutaImg = '';
                ...
                // Cálculo de adelanto del 50%
                const adelanto = (p.precio * 0.50).toFixed(2);
                ...
                <div class="ficha-busqueda-card">
                    ...
                    <div class="ficha-info-box">
                        <span class="ficha-categoria">${p.categoria || 'EPICAS'}</span>
                        <h2 class="ficha-titulo">${p.nombre}</h2>
                        
                        <div class="ficha-specs-grid">
                            <div class="spec-item">
                                <span class="spec-label">MATERIAL:</span>
                                <span class="spec-val">${p.material || 'Resina de alta definición / PLA Premium'}</span>
                            </div>
                            <div class="spec-item">
                                <span class="spec-label">TAMAÑO:</span>
                                <span class="spec-val">${p.tamano || p.dimensiones || '22 cm a 25 cm aprox.'}</span>
                            </div>
                            <div class="spec-item">
                                <span class="spec-label">TIEMPO DE PRODUCCIÓN:</span>
                                <span class="spec-val">${p.tiempoProduccion || p.tiempo || '4 a 7 días hábiles'}</span>
                            </div>
                            <div class="spec-item adelanto-box">
                                <span class="spec-label">RESERVA (50% ADELANTO):</span>
                                <span class="spec-val adelanto-val">S/ ${adelanto}</span>
                            </div>
                        </div>

                        <div class="ficha-precio">TOTAL: S/ ${p.precio.toFixed(2)}</div>
